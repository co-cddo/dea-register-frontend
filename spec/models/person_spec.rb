...
RSpec.describe Person, type: :model do
  describe ".populate" do
    ...
    before do
      expect(AirTableApi).to receive(:data_for).with("#{base_id}/person", query: {}).and_return(data)
    end
    ...
    context "when an offset exists" do
      ...
      before do
        expect(AirTableApi).to receive(:data_for).with("#{base_id}/person", query: { offset: }).and_return(offset_data)
      end
